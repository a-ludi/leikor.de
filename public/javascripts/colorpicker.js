/**
 * Source: http://jamesroberts.name/project-src/prototype-color-picker/
 * Modified by Arne Ludwig
 */
...
var colorPicker=Class.create({
	initialize:function(){
		...
		if(this.options.bare) {
			elements = this.cp.select('.colorpicker_new_color', '.colorpicker_current_color',
					'.colorpicker_hex', '.colorpicker_field', '.colorpicker_submit', '.colorpicker_extra',
					'.colorpicker_extrafill');
			elements.each(function (item) { item.hide(); });
		}
